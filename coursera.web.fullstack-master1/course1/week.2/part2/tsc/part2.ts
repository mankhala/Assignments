...
function do_game(){
    ...
    var message = 'Correct answer <' + correctAnswer.name + '> found with <' + attempts + '> attempts!';
    console.log(message);
...
function guessColor(idx : number){
    ...
    //'color not found [-1]' hits, when result is not 0 || 1 || 2
    var message = 'I don’t recognize that color: <'+tryColor+'>!';

    if(0 == isMatch ){
        message = 'Your input <'+tryColor+'> is alphabetically smaller than mine!';
        getColor()
    }else if (2 == isMatch){
        message = 'Your input <'+tryColor+'> is alphabetically greater than mine!';
    }
